...
import {
  fetchMissionRatings,
  MISSION_RATING_LABEL,
  MISSION_RATING_SYMBOL,
  type MissionRating,
  type MissionRatingEntry,
...
const POSITIVE: MissionRating[] = ["great", "good"];

function isPositive(r: MissionRating): boolean {
...
function groupBy<T extends string>(
  entries: MissionRatingEntry[],
  keyOf: (e: MissionRatingEntry) => T,
  labelOf: (key: T) => string
): GroupRow[] {
  const map = new Map<string, MissionRatingEntry[]>();
...
export function DevRatingsView() {
  ...
  const [entries, setEntries] = useState<MissionRatingEntry[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const rows = await fetchMissionRatings(supabase);
      if (cancelled) return;
      setEntries(rows);
      setLoadState("ready");
    }
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      bySituation: groupBy(
        ...
        (key) => (key === "unknown" ? "不明" : SITUATION_LABEL[key as Situation])
      ),
      byFeeling: groupBy(
        entries,
        (e) => (e.feeling ?? "unknown") as Feeling | "unknown",
        (key) => (key === "unknown" ? "不明" : FEELING_LABEL[key as Feeling])
      ),
      byMission: groupBy(
        entries,
        (e) => e.mission_id,
  ...
  if (loadState === "not-signed-in") {
    return (
      <p className="mt-10 text-sm text-ink-soft/70">
        本人アカウントでログインしていません。/login からログインしてください
        （Supabase側のRLSにより、本人以外はログインしても評価データを見られません）。
      </p>
    );
  }
    ...
    <div className="mt-10">
      ...
      <section className="mt-12">
        ...
        <ul className="mt-4 flex flex-col gap-3">
          {entries.map((e) => (
            <li key={e.id} className="rounded-2xl border border-line/70 p-4">
              <div className="flex items-center justify-between">
                <span className="text-xs text-ink-soft/60">{formatDateTime(e.created_at)}</span>
                <span className="text-sm text-sage-deep">
                  {MISSION_RATING_SYMBOL[e.rating]}{" "}
                  {MISSION_RATING_LABEL[e.rating].replace(/^[◎○△×]\s*/, "")}
                </span>
              </div>
              <p className="mt-2 text-sm text-ink">{e.mission_text}</p>
